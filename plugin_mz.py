...
class FLog(object):
    """
        Basic file logging - into /var/tmp/w2p_<application>.log

        from plugin_mz import flog, fLog
        fLog.setLevel('debug')   # default: LOG_LEVEL_DEFAULT ~ .WARNING 
        flog('aaa', 'critical')
        flog('bbb')  # DEBUG
    """
    def __init__(self, level=None, request=None):
        if request is None:
            request = current.request
        app = request.application
        self.logger = logging.getLogger(app)
        hnd = logging.handlers.TimedRotatingFileHandler(
            '/var/tmp/w2p_%s.log' % app,
            when='d',
            interval=1,
            backupCount=5,
        )
    ...
    def setLevel(self, level=None):
        """
        Args:
            level: logging.DEBUG or 'DEBUG' or 'debug', ... 
        """
        if level and type(level) == str:
            level = level.upper()
        self.logger.setLevel(level or LOG_LEVEL_DEFAULT)

fLog = FLog()

def flog(msg, level='debug'):
    level = level.lower()
    fc = getattr(fLog.logger, level, fLog.logging.DEBUG)
    fc(msg)
